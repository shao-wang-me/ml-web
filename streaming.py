...
if __name__ == '__main__':
    ...
    class MyStreamListener(StreamListener):
        ...
        def on_status(self, status):
            ...
            # run preprocessor via ProcessPoolExecutor
            process_pool.submit(fn=preprocessor.preprocess,
                                args=([status.text, status.created_at, status.user.screen_name],))

            # preprocessor.preprocess([status.text, status.created_at, status.user.screen_name])
    ...
    my_stream.filter(track=track_words, languages=['en'], stall_warnings=True)
